...
from bindenergy.data.constants import RESTYPE_1to3, RES_ATOM14, ALPHABET
...
class DrugAllAtomEnergyModel(FARigidModel):
    ...
    def __init__(self, args):
        ...
        self.density = [_density(exp, self.theta_range) for exp in self.expansion]
        self.score = [_score(exp, self.theta_range, sigma) for exp, sigma in zip(self.expansion, self.sigma_range)]
        self.register_buffer("eye", torch.eye(3).unsqueeze(0).float())
    ...
    def virtual_screen(self, protein_pdb, sdf_list, batch_size=200):
        pdb_file = pdb_io.PDBFile.read(protein_pdb)
        array = pdb_io.get_structure(pdb_file, model=1)
        array = array[struc.filter_amino_acids(array)]
        hseq = ""
        hcoords = []
        for res_id, res_name in zip(*struc.get_residues(array)):
            if res_name not in RESTYPE_3to1:
                continue
            aaname = RESTYPE_3to1[res_name]
            hseq += aaname
            aid = ALPHABET.index(aaname)
            res_coords = np.zeros((14, 3))
            res_atoms = array[(array.res_id == res_id) & (array.res_name == res_name)]
            for j, atom_name in enumerate(RES_ATOM14[aid]):
                if atom_name != '':
                    matches = res_atoms[res_atoms.atom_name == atom_name]
                    if len(matches) > 0:
                        res_coords[j] = matches.coord[0]
            hcoords.append(res_coords)
