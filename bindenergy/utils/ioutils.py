...
def load_esm_embedding(data, fields):
    _LOCAL_ESM_CKPT = r"C:/Users/hajmo/.cache/torch/hub/checkpoints/esm2_t36_3B_UR50D.pt"
    # Load the checkpoint directly to avoid PyTorch 2.6 weights_only restriction and
    # to skip the companion contact-regression file that the ESM library looks for
    # (DSMBind only needs sequence embeddings, not contact prediction).
    model_data = torch.load(_LOCAL_ESM_CKPT, map_location="cpu", weights_only=False)
    model, alphabet = esm.pretrained.load_model_and_alphabet_core(
        "esm2_t36_3B_UR50D", model_data, regression_data=None
    )
